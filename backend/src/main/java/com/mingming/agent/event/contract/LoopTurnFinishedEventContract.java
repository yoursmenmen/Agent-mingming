...
@Component
public class LoopTurnFinishedEventContract implements RunEventContract {

    private static final String CORRECTIONS_FIELD = "_contractCorrections";

    @Override
    public RunEventType eventType() {
        return RunEventType.LOOP_TURN_FINISHED;
    }
    ...
    public ObjectNode normalize(ObjectNode payload) {
        ...
        int consecutiveToolFailures = payload.path("consecutiveToolFailures").asInt(0);
        if (consecutiveToolFailures < 0) {
            corrections.add("consecutiveToolFailures");
        }

        payload.put("schemaVersion", "loop-turn-finished.v1");
        payload.put("turnIndex", Math.max(1, turnIndex));
        payload.put("elapsedMs", Math.max(0L, elapsedMs));
        payload.put("finalAnswerReady", payload.path("finalAnswerReady").asBoolean(false));
        payload.put("toolFailure", payload.path("toolFailure").asBoolean(false));
        payload.put("consecutiveToolFailures", Math.max(0, consecutiveToolFailures));
        return payload;
    }
    ...
    public List<String> validate(ObjectNode payload) {
        ...
        if (payload.path("consecutiveToolFailures").asInt(-1) < 0
                || hasCorrection(payload, "consecutiveToolFailures")) {
            errors.add("consecutiveToolFailures must be >= 0");
        }
