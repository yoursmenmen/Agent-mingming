...
class McpControllerTest {
    ...
    void listTools_shouldDelegateToService() {
        McpController controller = new McpController(mcpToolService);
    ...
    void callTool_shouldDelegateToService() {
        ...
        when(mcpToolService.callTool("docs", "search_docs", Map.of("query", "mcp"))).thenReturn(expected);

        Object result = controller.callTool(request);

        assertThat(result).isEqualTo(expected);
        verify(mcpToolService).callTool("docs", "search_docs", Map.of("query", "mcp"));
    }
