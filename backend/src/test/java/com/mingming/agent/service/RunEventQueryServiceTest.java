...
class RunEventQueryServiceTest {
    ...
    void getSessionEvents_shouldAggregateEventsAcrossRunsInOrder() {
        RunEventQueryService service = new RunEventQueryService(runEventRepository, agentRunRepository);
        ...
        List<Map<String, Object>> result = service.getSessionEvents(sessionId);
    ...
    @Test
    void getSessionEvents_shouldReturnEmptyWhenSessionHasNoRuns() {
        RunEventQueryService service = new RunEventQueryService(runEventRepository, agentRunRepository);
        UUID sessionId = UUID.randomUUID();
        when(agentRunRepository.findBySessionIdOrderByCreatedAtAsc(sessionId)).thenReturn(List.of());
