...
- [x] 写失败测试：状态查询与触发接口返回结构。
- [x] 运行失败测试并确认失败原因正确。
- [x] 实现最小功能（`GET status` + `POST trigger`）。
- [x] 运行测试确认通过。
...
- [x] 写失败测试：`running -> completed/failed` 状态流转。
- [x] 实现生命周期状态写入与 `RAG_SYNC` 事件。
- [x] 验证失败分支不会中断主服务。
...
- [x] 写失败测试：URL 源抓取、清洗、chunk 输出。
- [x] 实现 URL 抓取与文本归一化。
- [x] 接入现有 chunk/embedding 增量同步。
...
- [x] 增加 `RAG_SYNC` 与同步状态摘要展示。
- [x] 更新配置文档与项目进度。
- [x] 跑全量验证：backend tests + frontend test/build/lint。
