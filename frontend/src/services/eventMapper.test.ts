...
describe('mapRunEventToTimelineItem', () => {
  ...
  it('builds loop turn started summary with turn details', () => {
    const item = mapRunEventToTimelineItem({
      id: 'evt-loop-1',
      runId: 'run-1',
      seq: 9,
      createdAt: '2026-04-04T10:00:08Z',
      type: 'LOOP_TURN_STARTED',
      payload: JSON.stringify({
        turnIndex: 2,
        maxTurns: 5,
      }),
    ...
    expect(item.summary).toContain('循环开始')
    expect(item.summary).toContain('第 2 轮 / 共 5 轮')
  })

  it('builds loop turn finished summary with duration', () => {
    const item = mapRunEventToTimelineItem({
      id: 'evt-loop-2',
      runId: 'run-1',
      seq: 10,
      createdAt: '2026-04-04T10:00:09Z',
    ...
    expect(item.summary).toContain('循环结束')
    expect(item.summary).toContain('第 2 轮')
  ...
  it('builds loop terminated summary with reason and elapsedMs', () => {
    const item = mapRunEventToTimelineItem({
      ...
      runId: 'run-1',
      seq: 11,
      createdAt: '2026-04-04T10:00:10Z',
